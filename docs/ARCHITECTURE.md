...
3. **어제와 거의 판박이** → `topic_overlaps(엄격: jac≥0.34 or 4개)`로 최근 기록과 비교.
   **진전 있는 후속 기사는 통과**(엄격 임계값이라).

### 3건이 안 채워지는 문제 대응 (thin post 방지)
당일 주제 겹침으로 걸러진 후보는 **바로 버리지 않고 `spares`에 보관**한다. 루프가 끝났을 때
서로 다른 주제로 `NEWS_PER_CATEGORY`(=3)를 못 채웠으면 `spares`(화제성 순)에서 보충한다
→ "비슷한 1건"보다 "약간 관련 있어도 3건"이 낫다. 추가로 넓힘: **수집 시간창 20h→28h,
카테고리당 키워드 6→12개, MAX_EXAMINE = NEWS_PER_CATEGORY+10**.
그래도 심야엔 신선 뉴스가 적어 가끔 얇을 수 있으나 이전보다 3건 채워질 확률이 높아짐.
