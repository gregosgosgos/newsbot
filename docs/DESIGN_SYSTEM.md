...
## 표지 (cover) — 항상 통일된 레이아웃

- `render_cover_photo` **하나로 통일**. 레이아웃(제목 "오늘의 {cat} 뉴스" + 후킹 + 헤드라인 3건 + CTA)은
  사진 유무와 관계없이 **항상 동일**. 배경만 달라진다:
  - **사진 있음**: 그날 뉴스 중 좋은 자료사진이 있는 첫 뉴스 사진을 전면 배경으로(브랜드 네이비 컬러그레이드 + 스크림).
  - **사진 없음**: 딥네이비 그라디언트 + accent 글로우 + **카테고리 아이콘 워터마크**(`_fa_icon_a`) + 스파클.
- `generate_carousel`은 항상 `render_cover_photo(..., cover_photo, ...)` 호출(`cover_photo`가 ''일 수 있음).
- `render_cover`(구 확성기 3D 표지)는 **더 이상 사용 안 함**(코드는 남아있지만 호출되지 않음).
  이유: 사진 유무에 따라 확성기/사진 표지가 들쭉날쭉 → "옛날형식으로 돌아갔다"는 인상. 레이아웃 통일로 해결.
